...
//     const whatsappEspecialista = encodeURIComponent(
//         "Hola, quiero hablar con un especialista sobre el desarrollo de mi tienda online."
//     );

//     const whatsappMejorarTienda = encodeURIComponent(
//         "Hola, tengo una tienda online y quiero mejorar su rendimiento y ventas."
//     );

//     const whatsappConsultaGeneral = encodeURIComponent(
//         "Hola, tengo una consulta sobre desarrollo ecommerce."
//     );
...
export const Ecommerce = () => {
  // ======================
  // WhatsApp config (mensajes limpios + encode)
  // ======================
  const WA_PHONE = '5493532679392'

  const waHero = encodeURIComponent(
    'Hola, quiero asesoramiento para desarrollar o mejorar mi tienda online. ¿Cómo seguimos?'
  )

  const waMejorarTienda = encodeURIComponent(
    'Hola, tengo una tienda online y quiero mejorar su rendimiento y ventas.'
  )

  const waConsultaRapida = encodeURIComponent(
    'Hola, tengo una consulta sobre ecommerce y diseño web.'
  )

  const waEnterprise = encodeURIComponent(
    'Hola, necesito una solución enterprise para mi ecommerce. ¿Podemos coordinar una llamada?'
  )
  ...
  useEffect(() => {
    const serviceSchema = {
      ...
      "provider": { "@type": "Organization", "name": "Arvix" }
    }
  ...
  return (
    <div className="relative bg-black text-white overflow-x-hidden">
      ...
      <header className="relative pt-40 pb-20 border-b border-brand-border grid-bg">
        <div className="container mx-auto px-6">
          <div className="max-w-5xl">
            <span className="inline-block text-brand-lime px-3 py-1 text-[10px] font-bold uppercase tracking-widest mb-6">
              Ecommerce de Próxima Generación
            </span>
            ...
            <div className="flex flex-wrap gap-4">
              {/* CTA principal: Retener en sitio -> Contacto (misma pestaña) */}
              <Link
                to="/Contacto"
                aria-label="Solicitar asesoramiento para desarrollo ecommerce"
                className="inline-flex gap-3 bg-brand-lime text-black px-8 py-5 font-bold uppercase text-xs tracking-widest hover:-translate-y-1 hover:bg-brand-lime transition-all"
              >
                Hablar con un especialista <ChevronRight className="w-4 h-4" />
              </Link>

              {/* CTA secundario: WhatsApp (intención de mejora) */}
              <a
                href={`https://wa.me/${WA_PHONE}?text=${waMejorarTienda}`}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Consulta por mejora de tienda online vía WhatsApp"
                className="border border-brand-border text-white px-8 py-5 font-bold uppercase text-xs tracking-widest hover:bg-white/5 transition-all"
              >
                Quiero mejorar mi tienda
              </a>
            </div>
          </div>
        </div>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="relative bg-brand-black border border-brand-border p-4 rounded shadow-2xl overflow-hidden group">
              ...
              <div className="mt-6 text-right">
                {/* Iniciar Ahora -> Retener en sitio con plan preseleccionado */}
                <Link
                  to={`/Contacto?plan=${encodeURIComponent('Pack Full: Tienda + Benchmark + 30 Días')}`}
                  aria-label="Iniciar contacto para Pack Full"
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="border border-brand-border p-8 hover:border-brand-lime transition-all group flex flex-col justify-between">
              ...
              <a
                href={`/Contacto?plan=${encodeURIComponent('Start-Up Pack')}`}
                aria-label="Seleccionar Start-Up Pack"
                className="mt-12 w-full border border-brand-border py-4 text-[10px] font-bold uppercase tracking-widest group-hover:bg-white group-hover:text-black transition-all text-center block"
              >
                Seleccionar
              </a>
            </div>
            ...
            <div className="border-2 border-brand-lime p-10 relative bg-brand-gray/20 flex flex-col justify-between">
              ...
              <a
                href={`/Contacto?plan=${encodeURIComponent('Professional Redesign')}`}
                aria-label="Comenzar Professional Redesign"
                className="mt-12 w-full bg-brand-lime text-black py-4 text-[10px] font-bold uppercase tracking-widest hover:bg-white transition-all text-center block"
              >
                Comenzar Ahora
              </a>
            </div>
            ...
            <div className="border border-brand-border p-8 hover:border-brand-lime transition-all group flex flex-col justify-between">
              ...
              {/* Enterprise -> contacto por WhatsApp (external, nueva pestaña) */}
              <a
                href={`https://wa.me/${WA_PHONE}?text=${waEnterprise}`}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Consultar Enterprise por WhatsApp"
                className="mt-12 w-full border border-brand-border py-4 text-[10px] font-bold uppercase tracking-widest group-hover:bg-white group-hover:text-black transition-all text-center block"
              >
                Consultar
              </a>
      ...
      {/* ======================
          CTA FINAL
      ====================== */}
      <section className="py-24 border-t border-brand-border text-center">
        <a
          href={`https://wa.me/${WA_PHONE}?text=${waConsultaRapida}`}
          target="_blank"
          rel="noopener noreferrer"
